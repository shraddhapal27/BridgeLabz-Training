...
public class Employeeoperations {
	...
	list.add(new Employee(4,"Shraddha",21,"Female","IT",2025,40000.0));
...
//				5. Get the names of all employees who have joined after 2015 ?
	         list.stream().filter(e -> e.year > 2015).map(e -> e.name).forEach(System.out::println);
	         System.out.println("==============");
...
//		8. Get the details of youngest male employee in the product development department ?
      Optional<Employee> minAge = list.stream().filter(i -> (i.department.equals("IT") && i.gender.endsWith("Male"))).collect(Collectors.minBy(Comparator.comparing(Employee::getAge)));
         minAge.ifPresent(System.out::println);
	         
//	9. Who has the most working experience in the organization?
		Optional<Employee> maxYear = list.stream().collect(Collectors.minBy(Comparator.comparing(Employee::getYear)));
